...
5. **前提** Slack認証がまだ完了していない、**実行** 「Slackに共有」コマンドを実行する、**結果** Slack認証フローが開始され、ブラウザでOAuth認証ページが開く
...
### ユーザーストーリー 4 - Slack認証とワークスペース管理 (優先度: P3)

開発者が初回利用時にSlackワークスペースとの連携を設定し、複数のワークスペースを切り替えながら利用できます。認証情報は安全に管理され、再認証が必要な場合には自動的に通知されます。

**この優先度の理由**: 認証とワークスペース管理は重要ですが、初回設定後は頻繁に操作しないため、P3としています。基本的な共有・インポート機能の実装が優先されます。

**独立したテスト**: 初回利用時にSlack認証フローを完了し、複数のワークスペースを切り替えることで完全にテストでき、柔軟なワークスペース管理の価値を提供します。

**受け入れシナリオ**:

1. **前提** 開発者が初めてSlack統合機能を使用する、**実行** 「Slackに共有」コマンドを実行する、**結果** ブラウザでSlack OAuth認証ページが開く
2. **前提** OAuth認証ページが表示されている、**実行** ワークスペースを選択し、アクセスを許可する、**結果** VS Codeに戻り、認証完了メッセージが表示される
3. **前提** 複数のSlackワークスペースにアクセスできる、**実行** 「Slackワークスペースを切り替え」コマンドを実行する、**結果** 利用可能なワークスペース一覧が表示され、選択できる
4. **前提** Slackアクセストークンの有効期限が切れた、**実行** 共有またはインポートを実行する、**結果** 再認証が必要である旨の通知が表示され、再認証フローが開始される
5. **前提** 認証情報が保存されている、**実行** VS Codeを再起動する、**結果** 再認証なしで引き続きSlack統合機能が利用できる
...
- **FR-019**: システムは、初回利用時にSlack OAuthフローを実行し、ユーザー認証を完了しなければならない
- **FR-020**: システムは、Slack App DirectoryにSlack Appを公開し、ユーザーが簡単にインストールできるようにしなければならない
- **FR-021**: システムは、ワークフローファイルをSlackメッセージの添付ファイルとして保存しなければならない(外部ストレージへの依存を最小化する)
- **FR-022**: システムは、取得したSlackアクセストークンを安全に保存しなければならない(平文での保存は禁止)
- **FR-023**: システムは、アクセストークンの有効期限切れを検出し、再認証が必要な場合はユーザーに通知しなければならない
- **FR-024**: システムは、複数のSlackワークスペースを切り替えて利用できるようにしなければならない
- **FR-025**: システムは、現在アクティブなSlackワークスペース名をVS Code UIに表示しなければならない
...
- **Slack認証情報**: OAuthアクセストークン、リフレッシュトークン、有効期限、ワークスペースID、ワークスペース名。安全に暗号化して保存される。
