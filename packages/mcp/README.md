...
| `patch_workflow` | Structural edits without a full resend: `addNodes`, `removeNodeIds`, `addConnections`, `removeConnectionIds` (removals apply first, so a removed ID can be reused to replace). Removing a node cascades to its connections (`cascadedConnectionIds`); removing a group re-parents its children with their visual position preserved (`detachedNodeIds`). Validates the merged result before persisting; honours the same optimistic `revision` as `update_nodes`. |
| `render_workflow` | Read-only render of the current workflow: `format: "mermaid"` (default) returns just the fenced Mermaid flowchart block for pasting into a chat reply; `format: "md"` returns the full Markdown document `ccwf render` prints (title, description, diagram, execution instructions), with optional `agent` phrasing the instructions for that target. |
