...
/**
 * A single step in a guided workflow tour.
 *
 * Tours walk a reader through a workflow in a meaningful order (usually
 * execution order), spotlighting one or more nodes per step with a short
 * narration. Typically generated by an AI agent (e.g. when importing a
 * published Agent Skill as a workflow) and rendered by the canvas tour player.
 */
export interface TourStep {
  /** 1-based position of this step in the tour */
  order: number;
  /** Short step title */
  title: string;
  /** Narration explaining what these nodes do and why */
  description: string;
  /** IDs of the workflow nodes spotlighted in this step */
  nodeIds: string[];
  /** Optional teaching note about a concept/pattern surfaced by this step */
...
export interface Workflow {
  ...
  /** Optional guided tour walking a reader through this workflow */
  tour?: TourStep[];
}
