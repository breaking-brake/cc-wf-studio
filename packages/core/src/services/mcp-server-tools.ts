...
import type { McpServerManager } from './mcp-server-service.js';
import { getDefaultSchemaPath, loadWorkflowSchemaToon } from './schema-loader-service.js';
    ...
    async ({ workflow: workflowJson, description }) => {
      try {
        ...
        const applied = await manager.applyWorkflowToCanvas(
          parsedWorkflow as import('../types/workflow-definition.js').Workflow,
          description
        );
