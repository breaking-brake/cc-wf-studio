...
/**
 * Options for validateAIGeneratedWorkflow.
 */
export interface ValidateOptions {
  /**
   * Maximum number of nodes allowed. Defaults to VALIDATION_RULES.WORKFLOW.MAX_NODES
   * when omitted. The VSCode extension passes the user-configured value here.
   */
  maxNodes?: number;
}
 ...
 * @param options - Validation options (defaults applied when omitted)
...
export function validateAIGeneratedWorkflow(
  ...
  const maxNodes = options.maxNodes ?? VALIDATION_RULES.WORKFLOW.MAX_NODES;
  ...
  // Workflow does not declare a top-level `hooks` field (they live under
  // slashCommandOptions). Preserve the original behaviour while keeping tsc
  // happy — when the field is absent at runtime this branch simply skips.
  const maybeHooks = (wf as { hooks?: WorkflowHooks }).hooks;
  if (maybeHooks) {
    const hooksErrors = validateHooks(maybeHooks);
